"""Test KV Transfer for MewtwoCacheManager with PyNativeCacheTransceiver.

Uses threading + ThreadSafeDistributed to create PyNativeCacheTransceiver instances
in a single process. Validates all MewtwoAttentionType cache transfers across
...
# ---------------------------------------------------------------------------
# MewtwoCacheManager creation helpers
# ---------------------------------------------------------------------------
def _create_mewtwo_manager(
    mapping: Mapping,
    compress_ratios: List[int],
    dtype: DataType = DataType.BF16,
    compressor_dtype: DataType = DataType.FLOAT,
) -> MewtwoCacheManager:
    """Create a MewtwoCacheManager for the given mapping."""
...
def _create_managers_for_instance(
    ...
    enable_dp: bool,
    compress_ratios: List[int],
) -> List[MewtwoCacheManager]:
    """Create MewtwoCacheManagers for all ranks in an instance."""
    world_size = tp * pp
    managers = []
    for rank in range(world_size):
        mapping = Mapping(
        ...
        managers.append(_create_mewtwo_manager(mapping, compress_ratios))
    return managers
...
def _init_pool_data(managers: List, tp: int, seed_base: int = 0, fill_random: bool = True):
    ...
    Args:
        managers: List of MewtwoCacheManagers.
        tp: TP size.
...
def _read_cache_data(
    mgr: MewtwoCacheManager,
    layer_idx: int,
    attn_type: MewtwoAttentionType,
    request_id: int,
) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
    """Read cache data for a layer/attn_type from a MewtwoCacheManager.
...
def _find_ctx_rank_for_layer(
    layer_idx: int,
    ctx_managers: List[MewtwoCacheManager],
    ctx_tp: int,
    ctx_enable_dp: bool,
    req_idx: int,
) -> int:
...
def verify_all_requests(
    ...
    compress_ratios: List[int],
    ctx_managers: List[MewtwoCacheManager],
    gen_managers: List[MewtwoCacheManager],
    ctx_tp: int,
    ctx_pp: int,
    gen_tp: int,
    gen_pp: int,
...
def run_mewtwo_transfer_test(
    ...
    """Run a full mewtwo KV transfer test."""
...
def test_mewtwo_kv_transfer(
    ...
    """Test PyNativeCacheTransceiver with MewtwoCacheManager."""
    mode = "update_before" if update_before_transfer else "update_after"
    print(
        f"\nRunning mewtwo transfer test [{mode}]: "
...
def test_mewtwo_kv_transfer_uneven_pp(
    ctx_tp,
    ctx_pp,
    gen_tp,
    gen_pp,
    ...
    """Test PyNativeCacheTransceiver with MewtwoCacheManager and uneven layers-per-PP-rank.
    ...
    print(
        f"\nRunning mewtwo uneven PP transfer test [{mode}]: "
        f"ctx_tp={ctx_tp} ctx_pp={ctx_pp} gen_tp={gen_tp} gen_pp={gen_pp} "
        f"ctx_dp={ctx_enable_dp} gen_dp={gen_enable_dp} "
