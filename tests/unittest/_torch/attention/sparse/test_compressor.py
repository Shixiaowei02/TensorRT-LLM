...
from tensorrt_llm._torch.attention_backend.sparse.mewtwo.paged_kv_compress import (
    kv_compress_triton, kv_compress_prefill_triton, compressed_kv_scatter)
...
def run_pytorch_reference(
    new_kv, new_score, ape, kv_state, score_state,
    token_idx, compress_ratio, head_dim, overlap
):
    ...
    if overlap:
        ...
        if should_compress:
            d = head_dim
            kv_cat = torch.cat([kv_state[:bsz, :compress_ratio, :d], kv_state[:bsz, compress_ratio:, d:]], dim=1)
            score_cat = torch.cat([score_state[:bsz, :compress_ratio, :d], score_state[:bsz, compress_ratio:, d:]], dim=1)
...
def run_pytorch_prefill_reference(
    ...
    if overlap:
        ...
        score_transformed = torch.full((b, s, 2 * ratio, d), float("-inf"), device=score.device, dtype=score.dtype)
...
def run_pytorch_prefill_reference_varlen(
    kv_score, ape, kv_lens, start_pos, compress_ratio, head_dim, overlap,
    ...
    for b in range(bsz):
        ...
        if remainder > 0:
            ...
            score_state[b:b+1, offset : offset + remainder] = score[:, cutoff:] + ape[:remainder]
        ...
        if overlap:
            ...
            score_transformed = torch.full((1, s, 2 * ratio, d), float("-inf"), device=score.device, dtype=score.dtype)
...
def prepare_decode_metadata(
    ...
    cu_seq_lens = torch.arange(0, (batch_size + 1) * next_n, next_n,
                               device=device, dtype=torch.int32)
    cu_outputs = torch.arange(0, (batch_size + 1) * max_compressions, max_compressions,
                              device=device, dtype=torch.int32)
...
def create_paged_cache(batch_size, seqlen, compress_ratio, head_dim, overlap, page_size=4):
    ...
    block_table = torch.arange(num_blocks, device="cuda", dtype=torch.int32).view(batch_size, max_blocks)
...
def create_compressed_kv_cache(batch_size, max_compressed_len, head_dim, num_layers=1, tokens_per_block=4):
    ...
    kv_factor = 1
    kv_cache = torch.zeros(num_blocks, num_layers, kv_factor, tokens_per_block * head_dim, device="cuda")
    block_offsets = torch.zeros(1, batch_size, 2, max_blocks_per_seq, device="cuda", dtype=torch.int32)
    for b in range(batch_size):
        block_ids = torch.arange(b * max_blocks_per_seq, (b + 1) * max_blocks_per_seq, device="cuda", dtype=torch.int32)
...
def test_prefill_corner_cases(batch_size, seqlen, compress_ratio, head_dim, overlap):
    ...
    out_py = run_pytorch_prefill_reference(
        kv.clone(), score.clone(), ape, kv_state_py, score_state_py, compress_ratio, head_dim, overlap
    )
    ...
    kv_comp, compressed_mask = kv_compress_prefill_triton(
        kv_score, ape, kv_lens, start_pos,
        cu_seq_lens, cu_outputs,
    ...
    if out_py is None or num_chunks == 0:
        ...
        assert torch.allclose(out_py, out_tri_reshaped, rtol=1e-4, atol=1e-5), \
            f"Output mismatch: max diff = {(out_py - out_tri_reshaped).abs().max():.6f}"
...
def test_decode_corner_cases(batch_size, compress_ratio, head_dim, overlap, num_steps):
    ...
    for step in range(num_steps):
        ...
        out_py = run_pytorch_reference(
            ...
            kv_state_py, score_state_py, step, compress_ratio, head_dim, overlap
        )
        ...
        kv_comp, compressed_mask = kv_compress_triton(
            kv_score, ape, kv_lens, start_pos,
            cu_seq_lens, cu_outputs,
            paged_kv, paged_score, block_table, block_table,
        ...
        if should_compress:
            ...
            if out_py is not None:
                ...
                for b in range(batch_size):
                    ...
                    assert torch.allclose(out_py[b, 0, :head_dim], kv_comp[out_idx, :], rtol=1e-4, atol=1e-5), \
                        f"Step {step}, Batch {b}: mismatch diff={(out_py[b, 0, :head_dim] - kv_comp[out_idx, :]).abs().max():.6f}"
        else:
...
def test_prefill_state_update(batch_size, seqlen, compress_ratio, head_dim, overlap):
    ...
    _ = run_pytorch_prefill_reference(kv.clone(), score.clone(), ape, kv_state_py, score_state_py, compress_ratio, head_dim, overlap)
    ...
    kv_comp, _ = kv_compress_prefill_triton(
        kv_score, ape, kv_lens, start_pos,
        cu_seq_lens, cu_outputs,
        paged_kv, paged_score, block_table, block_table,
        compress_ratio, head_dim, overlap, page_size
...
def test_prefill_varlen(seq_lens_list, compress_ratio, head_dim, overlap):
    ...
    out_py = run_pytorch_prefill_reference_varlen(
        kv_score.clone(), ape, kv_lens, start_pos, compress_ratio, head_dim, overlap,
        kv_state_py, score_state_py
    )
    ...
    if total_actual_outputs == 0:
        ...
        if valid_outputs:
            ...
            assert torch.allclose(out_py, out_tri_valid, rtol=1e-4, atol=1e-5), \
                f"Output mismatch: max diff = {(out_py - out_tri_valid).abs().max():.6f}"
        else:
...
@pytest.mark.parametrize("batch_size,prefill_len,compress_ratio,head_dim,overlap,decode_steps", PREFILL_DECODE_CONFIGS)
def test_prefill_then_decode(batch_size, prefill_len, compress_ratio, head_dim, overlap, decode_steps):
    ...
    out_py_prefill = run_pytorch_prefill_reference(
        kv_prefill.clone(), score_prefill.clone(), ape,
        kv_state_py, score_state_py, compress_ratio, head_dim, overlap
    )
    ...
    kv_comp, compressed_mask = kv_compress_prefill_triton(
        kv_score_prefill, ape, kv_lens_prefill, start_pos_prefill,
        cu_seq_lens, cu_outputs,
        paged_kv, paged_score, block_table, block_table,
        compress_ratio, head_dim, overlap, page_size
    )
    ...
    if out_py_prefill is not None and kv_comp.numel() > 0:
        ...
        out_tri_reshaped = kv_comp.view(batch_size, num_chunks, head_dim)
        assert torch.allclose(out_py_prefill, out_tri_reshaped, rtol=1e-4, atol=1e-5), \
            f"Prefill output mismatch: {(out_py_prefill - out_tri_reshaped).abs().max():.6f}"
    ...
    for i in range(decode_steps):
        ...
        kv_comp, compressed_mask = kv_compress_triton(
            kv_score, ape, kv_lens, start_pos,
            cu_seq_lens_decode, cu_outputs_decode,
            paged_kv, paged_score, block_table, block_table,
            compress_ratio, head_dim, overlap, page_size, next_n=1
        )
        ...
        if should_compress and out_py is not None:
            ...
            for b in range(batch_size):
                ...
                assert torch.allclose(out_py[b, 0, :head_dim], kv_comp[out_idx, :], rtol=1e-4, atol=1e-5), \
                    f"Decode step {step}, Batch {b}: mismatch diff={(out_py[b, 0, :head_dim] - kv_comp[out_idx, :]).abs().max():.6f}"
...
def test_decode_mtp(batch_size, compress_ratio, head_dim, overlap, next_n):
    ...
    while step < num_steps:
        ...
        kv_comp, compressed_mask = kv_compress_triton(
            ...
            cu_seq_lens, cu_outputs,
            paged_kv, paged_score, block_table, block_table,
            compress_ratio, head_dim, overlap, page_size, next_n=actual_n
        )
        ...
        if len(py_outputs) > 0:
            ...
            for i, (token_idx, out_py) in enumerate(py_outputs):
                for b in range(batch_size):
                    ...
                    assert torch.allclose(out_py[b, 0, :head_dim], kv_comp[out_idx, :], rtol=1e-4, atol=1e-5), \
                        f"Token {token_idx}, Batch {b}: mismatch diff={(out_py[b, 0, :head_dim] - kv_comp[out_idx, :]).abs().max():.6f}"
...
SCATTER_CONFIGS = [
    # Basic cases
    pytest.param(1, 8, [4], [0], 4, 1, 0, id="single_batch_basic"),
    pytest.param(2, 16, [2, 3], [0, 1], 4, 1, 0, id="multi_batch_basic"),
    pytest.param(4, 32, [1, 2, 1, 3], [2, 0, 1, 4], 4, 1, 0, id="varied_outputs"),
    # Corner cases: single output
    pytest.param(1, 8, [1], [0], 4, 1, 0, id="single_output"),
    pytest.param(1, 16, [1], [5], 4, 1, 0, id="single_output_nonzero_start"),
    # Corner cases: cross block boundary
    pytest.param(1, 8, [6], [2], 4, 1, 0, id="cross_block_boundary"),
    pytest.param(2, 16, [5, 7], [3, 1], 4, 1, 0, id="multi_batch_cross_block"),
    # Corner cases: large head_dim
    pytest.param(1, 128, [3], [0], 4, 1, 0, id="large_head_dim"),
    pytest.param(2, 256, [2, 2], [0, 4], 4, 1, 0, id="very_large_head_dim"),
    # Corner cases: different tokens_per_block
    pytest.param(1, 16, [8], [0], 8, 1, 0, id="large_block_size"),
    pytest.param(2, 16, [3, 4], [0, 2], 2, 1, 0, id="small_block_size"),
    # Corner cases: multi-layer
    pytest.param(1, 16, [4], [0], 4, 4, 2, id="multi_layer_middle"),
    pytest.param(2, 32, [2, 3], [1, 0], 4, 8, 7, id="multi_layer_last"),
    # Corner cases: high start positions (stress paging)
    pytest.param(1, 16, [2], [100], 4, 1, 0, id="high_start_pos"),
    pytest.param(2, 8, [1, 1], [50, 75], 4, 1, 0, id="high_start_pos_multi"),
    # Corner cases: uneven batches
    pytest.param(3, 16, [1, 5, 2], [0, 0, 10], 4, 1, 0, id="uneven_outputs"),
    pytest.param(4, 8, [10, 1, 8, 3], [0, 5, 2, 12], 4, 1, 0, id="highly_uneven"),
]


@pytest.mark.parametrize(
    "batch_size,head_dim,num_outputs_list,start_positions,tokens_per_block,num_layers,layer_idx",
    SCATTER_CONFIGS
)
def test_compressed_kv_scatter(
    ...
    kv_cache, block_offsets, _, _ = create_compressed_kv_cache(
        batch_size, max_compressed_len, head_dim, num_layers, tokens_per_block
    )
    ...
    for b in range(batch_size):
        for i in range(num_outputs_list[b]):
            ...
            actual = kv_cache[phys_block, layer_idx, 0, token_offset * head_dim : (token_offset + 1) * head_dim]
