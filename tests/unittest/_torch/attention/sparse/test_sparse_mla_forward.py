...
from .test_compressor_module import RefCompressor
...
def calculate_reference_output_mewtwo_prefill(hidden_states,
    ...
    for req_idx in indices:
        ...
        if compress_ratio > 1:
            ...
            if num_compressed > 0:
                kv_combined = torch.cat([kv_seq, compressed_kv],
                                        dim=1).squeeze(0)
            else:
...
def calculate_reference_mewtwo_o_proj(attn_out_latent,
                                      o_a_proj,
                                      o_b_proj_weight,
                                      freqs_cis,
                                      n_local_groups,
    ...
    apply_rotary_emb(attn_out_latent[..., -qk_rope_head_dim:],
                     freqs_cis,
                     inverse=True)
    ...
    output = torch.nn.functional.linear(
        o_lora_flat, o_b_proj_weight)  # [num_tokens, hidden_size]

    return output


@pytest.mark.skipif(get_sm_version() < 90,
                    reason="Mewtwo attention requires SM90 (Hopper) or later")
...
def test_mewtwo_o_proj(num_tokens: int, dtype_str: str):
    ...
    with torch.no_grad():
        ...
        if dtype_str == "bf16":
            mla.o_a_proj.data = torch.randn(
                n_local_groups,
                o_lora_rank,
                num_heads * qk_head_dim // num_groups,
                dtype=dtype,
                device=device) * nn_init_std
        elif dtype_str == "fp8":
            dim = num_heads * qk_head_dim // num_groups
            o_a_proj_bf16 = torch.randn(n_local_groups,
                                        o_lora_rank,
                                        dim,
                                        dtype=torch.bfloat16,
                                        device=device) * nn_init_std

            fp8_a_weight, fp8_a_scale = per_block_cast_to_fp8_e8m0(
                o_a_proj_bf16.reshape(-1, dim))
            fp8_a_weight = fp8_a_weight.reshape(n_local_groups, o_lora_rank,
                                                dim)
        ...
        if dtype_str == "bf16":
            mla.o_b_proj.weight.data = torch.randn(hidden_size,
                                                   num_groups * o_lora_rank,
                                                   dtype=dtype,
                                                   device=device) * nn_init_std
        elif dtype_str == "fp8":
            # For FP8, properly quantize using fp8_quantize_1x128_sf_transpose
            o_b_proj_weight_bf16 = torch.randn(hidden_size,
                                               num_groups * o_lora_rank,
                                               dtype=torch.bfloat16,
                                               device=device) * nn_init_std

            # Quantize the weight
            fp8_b_weight, fp8_b_scale = per_block_cast_to_fp8_e8m0(
                o_b_proj_weight_bf16)
            fp8_b_weight_dequant = weight_dequant(fp8_b_weight,
                                                  fp8_b_scale).bfloat16()
    ...
    attn_out_latent = torch.randn(num_tokens,
                                  num_heads,
                                  qk_head_dim,
                                  dtype=dtype,
                                  device=device)
    ...
    freqs_cis = precompute_freqs_cis(
        qk_rope_head_dim, num_tokens, max_position_embeddings,
        rope_config.rope_theta, rope_config.rope_scaling['factor'],
        rope_config.rope_scaling['beta_fast'],
        rope_config.rope_scaling['beta_slow']).to(device)
    ...
    assert torch.isfinite(
        reference_output).all(), "Reference output contains non-finite values"
    ...
    if dtype_str == "fp8":
        ...
        print(
            f"  ✓ Test passed for num_tokens={num_tokens}, dtype={dtype_str}\n")
