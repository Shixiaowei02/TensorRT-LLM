...
def pytorch_gate_forward(scores: torch.Tensor, 
                         bias: torch.Tensor = None,
                         input_ids: torch.Tensor = None,
                         tid2eid: torch.Tensor = None,
                         route_scale: float = 1.5,
                         is_hash: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
    """
...
class TestMoeGate:
    ...
    def test_gate_forward_non_hash(self, batch_size, route_scale):
        ...
        torch.ops.trtllm.gate_forward(
            scores, bias, input_ids_tensor, tid2eid_tensor,
            out_weights, out_indices,
            TOPK, route_scale, False
        )
        ...
        # Verify shapes
        assert out_weights.shape == (batch_size, TOPK), f"weights shape mismatch: {out_weights.shape}"
        assert out_indices.shape == (batch_size, TOPK), f"indices shape mismatch: {out_indices.shape}"
    ...
    def test_gate_forward_hash(self, batch_size, route_scale, vocab_size):
        ...
        # Call CUDA kernel
        torch.ops.trtllm.gate_forward(
            scores, bias, input_ids, tid2eid,
            out_weights, out_indices,
            TOPK, route_scale, True
        )
        
        # Compute reference
        ref_weights, ref_indices = pytorch_gate_forward(
            scores, input_ids=input_ids, tid2eid=tid2eid, 
            route_scale=route_scale, is_hash=True
        )
    ...
    def test_gate_forward_edge_cases(self):
        ...
        assert torch.allclose(out_weights.sum(dim=-1), torch.tensor([1.5], device='cuda'), rtol=1e-4)
