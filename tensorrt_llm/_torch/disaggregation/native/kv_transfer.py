...
class ResourceRegister:
    def __init__(self, instance_rank_info: InstanceRankInfo, instance_info: InstanceInfo):
    ...
    def get_peer_domain_ranks(self, peer_instance_info: InstanceInfo, peer_dp_rank: int):
        # return the mapping rank relationship between self and peer in asymmetric parallelism case

        if peer_instance_info.instance_name + str(peer_dp_rank) in self.peer_domain_ranks_cache:
            return self.peer_domain_ranks_cache[
        ...
        self.peer_domain_ranks_cache[peer_instance_info.instance_name + str(peer_dp_rank)] = (
            peer_domain_ranks
        )
        return peer_domain_ranks

    def get_kv_block_ptrs_extractor(self, peer_instance_rank_info: InstanceRankInfo):
...
class TransferSession:
    def __init__(
        self, kv_cache_manager: KVCacheManager, request_id: int, resource_register: ResourceRegister
    ):
...
class SendSession(TransferSession):
    ...
    def extract_trans_meta(self, dst_info: TransferGenSideReqInfo) -> TransSendMeta:
        pass
...
class Sender:
    ...
    def async_send(self, request: LlmRequest) -> SendSession:
        ...
        send_session = SendSession(self.kv_cache_manager, request_id, self.resource_register)
