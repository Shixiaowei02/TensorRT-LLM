...
class NixlTransferAgent(BaseTransferAgent):
    ...
    def submit_transfer_requests(self, request: TransferRequest) -> TransferStatus:
        ...
        handle = self.agent.initialize_xfer(
            request.op, src_xfer_descs, dst_xfer_descs, request.remote_name, request.sync_message
        )
...
class BaseDataReceiver(ABC):
    ...
    # def __init__(self, kv_cache_manager: KVCacheManager,config: CacheTransceiverConfig):
    @abstractmethod
    def submit_transfer_task(self, transfer_meta_data: TransReqMeta | TransReqRecvMeta):
        raise NotImplementedError
...
class DataSender(BaseDataSender):
    ...
    def submit_transfer_task(self, transfer_meta_data: TransReqMeta):
        print(f" data sender submit_transfer_task, transfer_meta_data:{transfer_meta_data}")
        ...
        if not hasattr(self, "_transfer_queue"):
            ...
            def _consumer():
                while True:
                    ...
                    if meta is None:
                        break
                    try:
                        print("start handle_transfer_task")
            ...
            self._background_thread = threading.Thread(target=_consumer, daemon=True)
            self._background_thread.start()
        self._transfer_queue.put(transfer_meta_data)

    def _handle_transfer_task(self, transfer_meta_data: TransReqMeta):
        print(f" enter _handle_transfer_task, transfer_meta_data:{transfer_meta_data}")
        assert len(transfer_meta_data.src_kv_ptrs) == len(transfer_meta_data.dst_kv_ptrs)
        assert len(transfer_meta_data.kv_sizes) == len(transfer_meta_data.src_kv_ptrs)
        ...
        src_kv_list = [
            ...
            for src_ptr, size in zip(transfer_meta_data.src_kv_ptrs, transfer_meta_data.kv_sizes)
        ]
        dst_kv_list = [
            (dst_ptr, size, self.device_id)
            for dst_ptr, size in zip(transfer_meta_data.dst_kv_ptrs, transfer_meta_data.kv_sizes)
        ]
        ...
        request = TransferRequest(
            TransferOp.WRITE, src_memory_descs, dst_memory_descs, transfer_meta_data.remote_name, ""
        )
        status = self.agent.submit_transfer_requests(request)
        sync_status = "SUCCESS"
        if not status.wait():
            sync_status = "FAILED"
            transfer_meta_data.future_for_session.set_exception(RuntimeError("Transfer failed"))
        ...
        if (
            self.session_id_to_count[transfer_meta_data.session_id]
    ...
    def _get_socket(self, peer_endpoint: str):
        if peer_endpoint not in self.socket_cache:
            self.socket_cache[peer_endpoint] = self.zmq_context.socket(zmq.DEALER)
...
class DataReceiver(BaseDataReceiver):
    def __init__(self, agent: NixlTransferAgent, device_id: int):
        ...
        self._background_thread = threading.Thread(target=self._loop_for_receive_state, daemon=True)
        self._background_thread.start()
    ...
    def submit_transfer_task(self, transfer_meta_data: TransReqMeta | TransReqRecvMeta):
        if transfer_meta_data.session_id not in self.session_id_to_count:
            ...
            self.session_id_to_future[transfer_meta_data.session_id] = (
                transfer_meta_data.future_for_session
            )
        else:
            assert (
                self.session_id_to_future[transfer_meta_data.session_id]
    ...
    def _loop_for_receive_state(self):
        while True:
            ...
            if self._message_is_termination(recv_message):
                ...
                raise ValueError(f" data receiver received unknown message type: {recv_message[0]}")
    ...
    def _handle_task_state(self, send_id: bytes, message: list[bytes]):
        ...
        if task_state == "SUCCESS":
            print(f"task state is success session_id_to_count: {self.session_id_to_count}")
            ...
            raise ValueError(f" session {session_id} received unknown task state: {task_state}")
...
class InstanceInfo:
    ...
    def from_zmq(cls, msg: list[bytes]):
        return cls(
            instance_name=msg[0].decode("ascii"),
...
class TransferSession:
    ...
    def extract_trans_meta(self, dst_info: TransferGenSideReqInfo) -> TransReqMeta:
        ...
        peer_instance_rank_info: InstanceRankInfo = (
            self.transfer_manager.get_peer_instance_rank_info(
        ...
        need_send = self._need_to_transfer(peer_domain_ranks, peer_instance_rank_info)
        kv_factor = 1 if self.instance_rank_info.is_mla else 2
        self_kv_block_size = (
            self.instance_rank_info.layer_num_per_pp[self.instance_rank_info.pp_rank]
            * kv_factor
            * self.instance_rank_info.kv_head_num_per_rank
            * self.instance_rank_info.tokens_per_block
            * self.instance_rank_info.dims_per_head
            * self.instance_rank_info.element_size
        )
        # save
        peer_kv_block_size = (
            peer_instance_rank_info.layer_num_per_pp[peer_instance_rank_info.pp_rank]
            * kv_factor
            * peer_instance_rank_info.kv_head_num_per_rank
            * peer_instance_rank_info.tokens_per_block
            * peer_instance_rank_info.dims_per_head
            * peer_instance_rank_info.element_size
        )
        ...
        src_kv_blocks_ptrs = [
            src_kv_ptr + self_kv_block_size * block_id for block_id in src_block_ids
        ]
        dst_kv_blocks_ptrs = [
            dst_kv_ptr + peer_kv_block_size * block_id for block_id in dst_block_ids
        ]
        print(
            f" src_kv_blocks_ptrs: {src_kv_blocks_ptrs}, "
            f"src_kv_blocks_ptrs.shape: {len(src_kv_blocks_ptrs)} "
            f"self_kv_block_size: {self_kv_block_size}, "
            f"dst_kv_blocks_ptrs: {dst_kv_blocks_ptrs}, "
            f"dst_kv_blocks_ptrs.shape: {len(dst_kv_blocks_ptrs)} "
        ...
        (
            ...
            dst_kv_blocks_size,
        ) = self._gen_kv_block_ptrs_for_all_layers(
    ...
    ) -> tuple[TransReqRecvMeta, list[int]]:
        peer_domain_ranks = self._get_peer_peer_target_ranks(peer_instance_info, peer_dp_rank)
        expect_count = len(peer_domain_ranks.ranks)
        if not self.first_extracted:
            self.first_extracted = True
            self.remain_count = len(peer_domain_ranks.ranks)
    ...
    def create_gen_side_transfer_req_info(
        self, transfer_req_info: TransferReqInfo, disagg_params: DisaggregatedParams
    ...
    def _need_to_transfer(
        self, peer_domain_ranks: peerDomainRanks, peer_instance_rank_info: InstanceRankInfo
    ):
        ...
        self_tp_size_per_dp_group = (
            self.instance_rank_info.tp_size // self.instance_rank_info.dp_size
            if self.instance_rank_info.enable_attention_dp
            else self.instance_rank_info.tp_size
        )
    ...
    def _gen_kv_block_ptrs_for_all_layers(
    ...
    ]:  # (kv_block_ptr, kv_block_size), #kv format ,nhd or hnd
        ...
        self_layer_num = self.instance_rank_info.layer_num_per_pp[self.instance_rank_info.pp_rank]
        peer_layer_num = peer_instance_rank_info.layer_num_per_pp[peer_instance_rank_info.pp_rank]
        kv_factor = 1 if self.instance_rank_info.is_mla else 2
        assert (
            src_kv_block_size
            == self_layer_num
        ...
        peer_tp_size_per_dp_group = (
            peer_instance_rank_info.tp_size // peer_instance_rank_info.dp_size
            if peer_instance_rank_info.enable_attention_dp
            else peer_instance_rank_info.tp_size
        )
        self_tprank_in_dp_group = self.instance_rank_info.tp_rank % self_tp_size_per_dp_group
        peer_tprank_in_dp_group = peer_instance_rank_info.tp_rank % peer_tp_size_per_dp_group
        is_duplicate_head = (
            self.instance_rank_info.kv_head_num_per_rank * self_tp_size_per_dp_group
            != peer_instance_rank_info.kv_head_num_per_rank * peer_tp_size_per_dp_group
        )
        write_all_head = (
            is_duplicate_head
            or self.instance_rank_info.is_mla
            or self_tp_size_per_dp_group == peer_tp_size_per_dp_group
        )
        ...
        if write_all_head:  # write all heads but pp size are different
            ...
            src_kv_blocks_transfer_ptrs = [
                src_kv_block_ptr + src_block_offset for src_kv_block_ptr in src_kv_block_ptrs
            ]
            dst_kv_blocks_transfer_ptrs = [
                dst_kv_block_ptr + dst_block_offset for dst_kv_block_ptr in dst_kv_block_ptrs
            ]
            return (
                src_kv_blocks_transfer_ptrs,
                fragment_size,
        ...
        if self_tp_size_per_dp_group < peer_tp_size_per_dp_group:
            src_head_offset = (
                peer_tprank_in_dp_group
                % (peer_tp_size_per_dp_group // self_tp_size_per_dp_group)
                * continue_heads_fragment_size
            )
            dst_head_offset = 0
        if self_tp_size_per_dp_group > peer_tp_size_per_dp_group:
        ...
        src_kv_blocks_transfer_ptrs = []
        dst_kv_blocks_transfer_ptrs = []
        src_layer_kv_ele_size = (
            self.instance_rank_info.kv_head_num_per_rank
            * self.instance_rank_info.tokens_per_block
            * self.instance_rank_info.dims_per_head
            * self.instance_rank_info.element_size
        )
        dst_layer_kv_ele_size = (
            peer_instance_rank_info.kv_head_num_per_rank
            * peer_instance_rank_info.tokens_per_block
            * peer_instance_rank_info.dims_per_head
            * peer_instance_rank_info.element_size
            ...
            for layer_id in range(transfer_layer_num):
                ...
                for kv in range(kv_factor):
                    src_kv_block_transfer_ptr = (
                        src_kv_block_ptr
                        + src_layer_ele_size * src_layer_id
                        + src_layer_kv_ele_size * kv
                        + src_head_offset
                    )
                    dst_kv_block_transfer_ptr = (
                        dst_kv_block_ptr
                        + dst_layer_ele_size * dst_layer_id
                        + dst_layer_kv_ele_size * kv
                        + dst_head_offset
                    )
                    src_kv_blocks_transfer_ptrs.append(src_kv_block_transfer_ptr)
                    dst_kv_blocks_transfer_ptrs.append(dst_kv_block_transfer_ptr)
        return (
            src_kv_blocks_transfer_ptrs,
            continue_heads_fragment_size,
            dst_kv_blocks_transfer_ptrs,
    ...
    def _get_peer_peer_target_ranks(
        self, peer_instance_info: InstanceInfo, peer_dp_rank: int
    ) -> peerDomainRanks:
        return self.transfer_manager.get_peer_domain_ranks(peer_instance_info, peer_dp_rank)
...
class CacheTransferManager:
    def __init__(self, instance_rank_info: InstanceRankInfo, kv_cache_manager: KVCacheManager):
        self.instance_rank_info = instance_rank_info
    ...
    def register_peer(
        self, peer_instance_name: str, peer_rank: int, peer_instance_info: InstanceRankInfo
    ):
        self.peer_instance_instance_info_cache[peer_instance_name + str(peer_rank)] = (
            peer_instance_info
        )

    # Get the cache info of the peer.
    def get_peer_instance_rank_info(
        self, peer_instance_name: str, peer_rank: int
    ) -> InstanceRankInfo:
        return self.peer_instance_instance_info_cache[peer_instance_name + str(peer_rank)]
    ...
    def create_transfer_session(
        ...
        session = TransferSession(
            transferReqInfo=src, transfer_manager=self, instance_rank_info=self.instance_rank_info
        )
    ...
    def create_trans_req_info(self, request: LlmRequest) -> TransferReqInfo:
        block_ids = self.kv_cache_manager.get_batch_cache_indices([request.py_request_id])[0]
    ...
    def get_peer_domain_ranks(
        self, peer_instance_info: InstanceInfo, peer_dp_rank: int
    ) -> peerDomainRanks:
        if (
            str(peer_instance_info.instance_name) + str(peer_dp_rank)
            in self._peer_domain_ranks_cache
        ):
        ...
        for pp_rank in range(peer_instance_info.pp_size):
            ...
            peer_pp_end_layer_id = (
                peer_pp_start_layer_id + peer_instance_info.layer_num_per_pp[pp_rank]
            )
            if (
                self_start_layer_id < peer_pp_end_layer_id
        ...
        peer_tp_size_per_dp_group = (
            peer_instance_info.tp_size / peer_instance_info.dp_size
        ...
        if self_tp_size_per_dp_group <= peer_tp_size_per_dp_group:
            ...
            peer_tp_rank_start = (
                self_tprank_in_dp_group * domain_tp_size + peer_dp_rank * peer_tp_size_per_dp_group
            )
            peer_tp_rank_end = peer_tp_rank_start + domain_tp_size
        else:
            peer_tp_rank_start = (
                self_tprank_in_dp_group // (self_tp_size_per_dp_group // peer_tp_size_per_dp_group)
        ...
        for pp_rank in range(peer_pp_rank_start, peer_pp_rank_end):
            for cp_rank in range(peer_cp_rank_start, peer_cp_rank_end):
                for tp_rank in range(peer_tp_rank_start, peer_tp_rank_end):
                    ranks.append(
                        pp_rank * peer_instance_info.tp_size * peer_instance_info.cp_size
                        + cp_rank * peer_instance_info.tp_size
                        + tp_rank
                    )
        duplicate_head_factor = max(
            ...
            self.instance_rank_info.kv_head_num_per_rank
            * self_tp_size_per_dp_group
            // (peer_instance_info.kv_head_num_per_rank * peer_tp_size_per_dp_group),
        )
        ...
        self._peer_domain_ranks_cache[str(peer_instance_info.instance_name) + str(peer_dp_rank)] = (
            peer_domain_ranks
        )
...
class Transceiver:
    ...
    def update_instance_info_ctx_server_endpoints(self, ctx_server_endpoints: list[str]):
        self.instance_info.ctx_server_endpoints = ctx_server_endpoints

    def async_send(self, request: LlmRequest) -> TransferSession:
        # return future object ?
        transfer_req_info = self.cache_transfer_manager.create_trans_req_info(request)
        transfer_session = self.cache_transfer_manager.create_transfer_session(transfer_req_info)
        with self.request_id_to_send_transfer_session_lock:
            self.request_id_to_send_transfer_session_cache[transfer_req_info.req_id] = (
                transfer_session
            )
    ...
    def _handle_send_transfer_session(self, transfer_session: TransferSession):
        ...
        for transfer_recv_req_info in transfer_recv_req_info_dict.values():
            trans_meta = transfer_session.extract_trans_meta(transfer_recv_req_info)
            self.data_sender.submit_transfer_task(trans_meta)
            if not transfer_session.is_active():
                with self.request_id_to_send_transfer_session_lock:
                    print(
                        " handle_send_transfer_session, delete "
                        f"request_id_to_send_transfer_session_cache: "
                        f"{transfer_session.request_id}"
                    )
                    if (
                        transfer_session.request_id
                        in self.request_id_to_send_transfer_session_cache
                    ):
    ...
    def _get_send_transfer_session(self, ctx_req_id: int) -> TransferSession:
        with self.request_id_to_send_transfer_session_lock:
            if ctx_req_id in self.request_id_to_send_transfer_session_cache:
                return self.request_id_to_send_transfer_session_cache[ctx_req_id]
            else:
                return None
    ...
    def _handle_request_data(self, send_id: bytes, message: list[bytes]):
        ...
        print(f" _handle_request_data, transfer_gen_side_req_info:{transfer_gen_side_req_info}")
        ...
        if send_transfer_session is None:
            ...
            if not send_transfer_session.is_active():
                with self.request_id_to_send_transfer_session_lock:
                    print(" handle_request_data, delete request_id_to_send_transfer_session_cache")

                    if ctx_req_id in self.request_id_to_send_transfer_session_cache:
                        del self.request_id_to_send_transfer_session_cache[ctx_req_id]
                ## TODO: manager clean
    ...
    def _async_request_data(self, request: LlmRequest):
        ...
        for rank in target_ranks:
            self._send_data_request(
                context_peer_infos.ctx_server_endpoints[rank], transfer_recv_req_info
            )
    ...
    def _get_context_info(self, disagg_params: DisaggregatedParams) -> InstanceInfo:
        if self._need_register_peer_in_first_request(disagg_params):
            ...
            print(
                f"get_context_info connect to ctx_leader_endpoint: {disagg_params.ctx_info_endpoint}"
            )
    ...
    def _send_data_request(self, endpoint: str, transfer_recv_req_info: TransferGenSideReqInfo):
...
def test_nixl_transfer_agent():
    ...
    src_memory_desc = [
        (src_tensor.data_ptr(), src_tensor.numel() * src_tensor.element_size(), 0),
        (src_tensor2.data_ptr(), src_tensor2.numel() * src_tensor2.element_size(), 0),
    ]
    dst_memory_desc = [
        (dst_tensor.data_ptr(), dst_tensor.numel() * dst_tensor.element_size(), 0),
        (dst_tensor2.data_ptr(), dst_tensor2.numel() * dst_tensor2.element_size(), 0),
    ]
    reg_scr_memory_desc = [
        (src_tensor.data_ptr(), src_tensor.numel() * src_tensor.element_size(), 0, "src1"),
        (src_tensor2.data_ptr(), src_tensor2.numel() * src_tensor2.element_size(), 0, "src2"),
    ]
    reg_dst_memory_desc = [
        (dst_tensor.data_ptr(), dst_tensor.numel() * dst_tensor.element_size(), 0, "dst1"),
        (dst_tensor2.data_ptr(), dst_tensor2.numel() * dst_tensor2.element_size(), 0, "dst2"),
    ]
    ...
    request = TransferRequest(TransferOp.WRITE, src_memory_descs, dst_memory_descs, "agent2", "")
...
def test_cache_transceiver():
    ...
    ctx_instance_rank_info = InstanceRankInfo(
        ...
        is_mla=False,
        layer_num_per_pp=[num_layers],
        kvcache_ptrs=[ctx_kv_cache_manager.get_unique_primary_pool().data_ptr()],
        aux_ptrs=[],
        server_endpoint="",
        recv_endpoint="",
        transfer_engine_info=bytes(),
    ...
    gen_instance_rank_info = InstanceRankInfo(
        ...
        is_mla=False,
        layer_num_per_pp=[num_layers],
        kvcache_ptrs=[gen_kv_cache_manager.get_unique_primary_pool().data_ptr()],
        aux_ptrs=[],
        server_endpoint="",
        recv_endpoint="",
        transfer_engine_info=bytes(),
    ...
    print(f" ctx instance info kv cache ptrs :{ctx_instance_rank_info.kvcache_ptrs}")
    print(f" gen instance info kv cache ptrs :{gen_instance_rank_info.kvcache_ptrs}")

    device_id = 0

    ctx_transceiver = Transceiver(
        ctx_kv_cache_manager, device_id, ctx_instance_info, ctx_instance_rank_info
    )
    gen_transceiver = Transceiver(
        gen_kv_cache_manager, device_id, gen_instance_info, gen_instance_rank_info
    )
    ...
    gen_request.py_disaggregated_params = DisaggregatedParams(
        ...
        ctx_dp_rank=0,
        ctx_info_endpoint=ctx_instance_info.ctx_server_endpoints[0],
    )
    ...
    print(
        f"ctx_block_datas: {ctx_block_datas}, "
        f"ctx_block_datas.shape: {ctx_block_datas.shape}, "
        f"ctx_block_datas.data_ptr: {ctx_block_datas.data_ptr()}"
    )
    print(
        f"gen_block_datas: {gen_block_datas}, "
        f"gen_block_datas.shape: {gen_block_datas.shape}, "
...
def test_cache_transceiver_with_tp(ctx_tp, gen_tp):
    ...
    for ctx in range(ctx_tp_size):
        ...
        ctx_pool = ctx_kv_cache_manager.get_unique_primary_pool()
        print(f" ctx cache pool ptr: {ctx_pool.data_ptr()}, ctx cache pool shape: {ctx_pool.shape}")
    ...
    for gen in range(gen_tp_size):
        ...
        gen_pool = gen_kv_cache_manager.get_unique_primary_pool()
        print(f" gen cache pool ptr: {gen_pool.data_ptr()}, gen cache pool shape: {gen_pool.shape}")
    ...
    for ctx in range(ctx_tp_size):
        ctx_transceiver = Transceiver(
            ctx_kv_cache_managers[ctx], device_id, ctx_instance_info, ctx_instance_rank_infos[ctx]
        )
        ctx_transceivers.append(ctx_transceiver)

    ctx_server_endpoints = []
    for ctx in range(ctx_tp_size):
        ctx_server_endpoints.append(ctx_transceivers[ctx].instance_rank_info.server_endpoint)

    for ctx in range(ctx_tp_size):
        ctx_transceivers[ctx].update_instance_info_ctx_server_endpoints(ctx_server_endpoints)

    gen_transceivers = []
    for gen in range(gen_tp_size):
        gen_transceiver = Transceiver(
            gen_kv_cache_managers[gen], device_id, gen_instance_info, gen_instance_rank_infos[gen]
        )
    ...
    for ctx in range(ctx_tp_size):
        ctx_kv_cache_managers[ctx].impl.add_sequence(
            ctx_request.py_request_id, ctx_request.prompt_len, 1, ctx_request
        )
    ...
    gen_request.py_disaggregated_params = DisaggregatedParams(
        ctx_request_id=ctx_request.py_request_id,
        ctx_dp_rank=0,
        ctx_info_endpoint=ctx_instance_rank_infos[0].server_endpoint,
    )

    for gen in range(gen_tp_size):
        gen_kv_cache_managers[gen].impl.add_sequence(
            gen_request.py_request_id, gen_request.prompt_len, 1, gen_request
        )
    ...
    for ctx in range(ctx_tp_size):
        result = send_sessions[ctx].get_future_for_session().result()
        print(f"send_sessions[{ctx}].get_future_for_session().result(): {result}")

    for gen in range(gen_tp_size):
        result = recv_sessions[gen].get_future_for_session().result()
        print(f"recv_sessions[{gen}].get_future_for_session().result(): {result}")

    ctx_data_tensors = []
    for ctx in range(ctx_tp_size):
        ctx_data_pool_tensor = ctx_kv_cache_managers[ctx].get_unique_primary_pool()
    ...
    gen_data_tensors = []
    for gen in range(gen_tp_size):
        gen_data_pool_tensor = gen_kv_cache_managers[gen].get_unique_primary_pool()
