    ...
    std::optional<torch::Tensor> scale_output)
{
    ...
    tk::postProcessScatterLaunch(kv_comp.data_ptr(), kv_out.has_value() ? kv_out->data_ptr() : nullptr,
        ...
        static_cast<int>(cache_mode), rotate_activation, fp8_output.has_value() ? fp8_output->data_ptr() : nullptr,
        scale_output.has_value() ? scale_output->data_ptr<float>() : nullptr, stream);
}
