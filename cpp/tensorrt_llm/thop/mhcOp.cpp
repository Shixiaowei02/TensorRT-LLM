    ...
    int64_t sinkhorn_repeat)
{
    ...
    tk::mhcBigFuseLaunch(
        ...
        r_acc.data_ptr<float>(),
        reinterpret_cast<__nv_bfloat16 const*>(residual.data_ptr<at::BFloat16>()),
    ...
    int64_t K)
{
    ...
    TORCH_CHECK(status == CUBLAS_STATUS_SUCCESS,
                "cublasGemmEx failed with status ", static_cast<int>(status));
}

void mhcHcHeadApplyOp(
    torch::Tensor mixes,
    torch::Tensor sqrsum,
    torch::Tensor x,
    torch::Tensor out,
    ...
    double eps)
{
    ...
    tk::mhcHcHeadApplyLaunch(
        mixes.data_ptr<float>(),
        sqrsum.data_ptr<float>(),
        reinterpret_cast<__nv_bfloat16 const*>(x.data_ptr<at::BFloat16>()),
        reinterpret_cast<__nv_bfloat16*>(out.data_ptr<at::BFloat16>()),
        scale.data_ptr<float>(),
        base_t.data_ptr<float>(),
        static_cast<int>(M),
