...
__global__ void mhcBigFuseKernel(
    float const* __restrict__ y_acc,
    float const* __restrict__ r_acc,
    ...
    int sinkhorn_repeat)
{
    ...
    if (warp_id == 0 && lane < n) {
        ...
        #pragma unroll
        for (int k = 0; k < n; k++)
            cm[k] = y_row[2 * n + lane * n + k] * rstd * s2
                  + hc_base[2 * n + lane * n + k];
    }
    ...
    if (warp_id > 0) {
        ...
        __nv_bfloat16 const* rbase =
            residual + static_cast<long long>(token) * n * hidden_size;
        __nv_bfloat16* obase =
            layer_input + static_cast<long long>(token) * hidden_size;
        ...
        for (int h = p2_tid * 8; h < hidden_size; h += p2_threads * 8) {
            ...
            #pragma unroll
            for (int j = 0; j < n; j++) {
                float const pm = (j == 0) ? pm0 : (j == 1) ? pm1
                               : (j == 2) ? pm2 : pm3;

                uint4 raw = *reinterpret_cast<uint4 const*>(
                                &rbase[j * hidden_size + h]);
                __nv_bfloat162 const* pairs =
                    reinterpret_cast<__nv_bfloat162 const*>(&raw);
                ...
                acc4 += pm * f2.x;  acc5 += pm * f2.y;
                acc6 += pm * f3.x;  acc7 += pm * f3.y;
            }

            uint4 out_raw;
            __nv_bfloat162* opairs =
                reinterpret_cast<__nv_bfloat162*>(&out_raw);
...
__global__ void mhcSqrsumKernel(
...
__launch_bounds__(256)
__global__ void mhcGemmSqrsumFmaKernel(
    __nv_bfloat16 const* __restrict__ X,
    float const* __restrict__ W_T,
    ...
    int M, int N, int K, int num_k_blocks, int k_chunk)
{
    ...
    for (int n_base = 0; n_base < N; n_base += N_TILE) {
        ...
        if (row_idx < M) {
            for (int k_base = k_start; k_base + K_STEP <= k_end; k_base += K_STEP) {
                ...
                for (int n = 0; n < N_TILE; n++) {
                    if (n_base + n < N) {
                        float w0, w1, w2, w3;
                        asm volatile(
                            "ld.global.L1::evict_last.v4.f32 {%0, %1, %2, %3}, [%4];"
                            : "=f"(w0), "=f"(w1), "=f"(w2), "=f"(w3)
...

            {
                ...
                for (int kk = tail_start + tid; kk < k_end; kk += 256) {
                    ...
                    if (n_base == 0) sqr = fmaf(xv, xv, sqr);
                    #pragma unroll
                    for (int n = 0; n < N_TILE; n++) {
                        if (n_base + n < N) {
        ...
        if (lane == 0 && row_idx < M && (n_base + warp_id) < N) {
            float val = s_warp[0][warp_id] + s_warp[1][warp_id]
                      + s_warp[2][warp_id] + s_warp[3][warp_id]
                      + s_warp[4][warp_id] + s_warp[5][warp_id]
                      + s_warp[6][warp_id] + s_warp[7][warp_id];
            if (single_k)
                y_row[n_base + warp_id] = val;
            else
                asm volatile("red.global.add.f32 [%0], %1;"
                             :: "l"(y_row + n_base + warp_id), "f"(val));

            if (n_base == 0 && warp_id == 0) {
                float sq = s_warp[0][N_TILE] + s_warp[1][N_TILE]
                         + s_warp[2][N_TILE] + s_warp[3][N_TILE]
                         + s_warp[4][N_TILE] + s_warp[5][N_TILE]
                         + s_warp[6][N_TILE] + s_warp[7][N_TILE];
                if (single_k)
                    R[row_idx] = sq;
                else
                    asm volatile("red.global.add.f32 [%0], %1;"
...
__launch_bounds__(256)
__global__ void mhcPostMappingKernel(
    __nv_bfloat16 const* __restrict__ residual,
    __nv_bfloat16 const* __restrict__ x,
    float const* __restrict__ post_mix,
    float const* __restrict__ comb_mix,
    ...
    int hidden_size)
{
    ...
    for (int h = tid * 8; h < hidden_size; h += 256 * 8) {
        ...
        a0[0] = pm0 * xf0.x; a0[1] = pm0 * xf0.y;
        a0[2] = pm0 * xf1.x; a0[3] = pm0 * xf1.y;
        a0[4] = pm0 * xf2.x; a0[5] = pm0 * xf2.y;
        a0[6] = pm0 * xf3.x; a0[7] = pm0 * xf3.y;

        a1[0] = pm1 * xf0.x; a1[1] = pm1 * xf0.y;
        a1[2] = pm1 * xf1.x; a1[3] = pm1 * xf1.y;
        a1[4] = pm1 * xf2.x; a1[5] = pm1 * xf2.y;
        a1[6] = pm1 * xf3.x; a1[7] = pm1 * xf3.y;

        a2[0] = pm2 * xf0.x; a2[1] = pm2 * xf0.y;
        a2[2] = pm2 * xf1.x; a2[3] = pm2 * xf1.y;
        a2[4] = pm2 * xf2.x; a2[5] = pm2 * xf2.y;
        a2[6] = pm2 * xf3.x; a2[7] = pm2 * xf3.y;

        a3[0] = pm3 * xf0.x; a3[1] = pm3 * xf0.y;
        a3[2] = pm3 * xf1.x; a3[3] = pm3 * xf1.y;
        a3[4] = pm3 * xf2.x; a3[5] = pm3 * xf2.y;
        a3[6] = pm3 * xf3.x; a3[7] = pm3 * xf3.y;

        #pragma unroll
        for (int k = 0; k < n; k++) {
            uint4 r_raw = *reinterpret_cast<uint4 const*>(
                &residual[tok_res + k * hidden_size + h]);
            __nv_bfloat162 const* rp =
            ...
            float const ck0 = (k == 0) ? c00 : (k == 1) ? c10
                             : (k == 2) ? c20 : c30;
            float const ck1 = (k == 0) ? c01 : (k == 1) ? c11
                             : (k == 2) ? c21 : c31;
            float const ck2 = (k == 0) ? c02 : (k == 1) ? c12
                             : (k == 2) ? c22 : c32;
            float const ck3 = (k == 0) ? c03 : (k == 1) ? c13
                             : (k == 2) ? c23 : c33;

            a0[0] = fmaf(ck0, rf0.x, a0[0]); a0[1] = fmaf(ck0, rf0.y, a0[1]);
            a0[2] = fmaf(ck0, rf1.x, a0[2]); a0[3] = fmaf(ck0, rf1.y, a0[3]);
            a0[4] = fmaf(ck0, rf2.x, a0[4]); a0[5] = fmaf(ck0, rf2.y, a0[5]);
            a0[6] = fmaf(ck0, rf3.x, a0[6]); a0[7] = fmaf(ck0, rf3.y, a0[7]);

            a1[0] = fmaf(ck1, rf0.x, a1[0]); a1[1] = fmaf(ck1, rf0.y, a1[1]);
            a1[2] = fmaf(ck1, rf1.x, a1[2]); a1[3] = fmaf(ck1, rf1.y, a1[3]);
            a1[4] = fmaf(ck1, rf2.x, a1[4]); a1[5] = fmaf(ck1, rf2.y, a1[5]);
            a1[6] = fmaf(ck1, rf3.x, a1[6]); a1[7] = fmaf(ck1, rf3.y, a1[7]);

            a2[0] = fmaf(ck2, rf0.x, a2[0]); a2[1] = fmaf(ck2, rf0.y, a2[1]);
            a2[2] = fmaf(ck2, rf1.x, a2[2]); a2[3] = fmaf(ck2, rf1.y, a2[3]);
            a2[4] = fmaf(ck2, rf2.x, a2[4]); a2[5] = fmaf(ck2, rf2.y, a2[5]);
            a2[6] = fmaf(ck2, rf3.x, a2[6]); a2[7] = fmaf(ck2, rf3.y, a2[7]);

            a3[0] = fmaf(ck3, rf0.x, a3[0]); a3[1] = fmaf(ck3, rf0.y, a3[1]);
            a3[2] = fmaf(ck3, rf1.x, a3[2]); a3[3] = fmaf(ck3, rf1.y, a3[3]);
            a3[4] = fmaf(ck3, rf2.x, a3[4]); a3[5] = fmaf(ck3, rf2.y, a3[5]);
            a3[6] = fmaf(ck3, rf3.x, a3[6]); a3[7] = fmaf(ck3, rf3.y, a3[7]);
        }

        #pragma unroll
        for (int j = 0; j < n; j++) {
            float* aj = (j == 0) ? a0 : (j == 1) ? a1
                      : (j == 2) ? a2 : a3;
            uint4 o_raw;
            ...
            *reinterpret_cast<uint4*>(
                &out[tok_res + j * hidden_size + h]) = o_raw;
        }
    }
}
...
__launch_bounds__(256)
__global__ void mhcHcHeadApplyKernel(
    float const* __restrict__ mixes,
    float const* __restrict__ sqrsum,
    __nv_bfloat16 const* __restrict__ x,
    __nv_bfloat16* __restrict__ out,
    ...
    cudaStream_t stream)
{
    ...
    mhcBigFuseKernel<<<grid, block, 0, stream>>>(
...
{
    dim3 grid(static_cast<unsigned int>(M),
              static_cast<unsigned int>(num_k_blocks));
    dim3 block(256);
    ...
    mhcGemmSqrsumFmaKernel<<<grid, block, 0, stream>>>(
        x, w_t, y, r, M, N, K, num_k_blocks, k_chunk);
}
    ...
    cudaStream_t stream)
{
    ...
    mhcHcHeadApplyKernel<<<grid, block, 0, stream>>>(
        mixes, sqrsum, x, out, scale, base,
        mult, hidden_size, K, norm_eps, eps);
}
    ...
    cudaStream_t stream)
{
    ...
    mhcPostMappingKernel<<<grid, block, 0, stream>>>(
        residual, x, post_mix, comb_mix, out, hidden_size);
}
